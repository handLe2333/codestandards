    ...
    function u(e) {
        ...
        return function r(e, i) {
            for (var a = e.firstChild; a; a = a.nextSibling) 3 === a.nodeType ? i += a.nodeValue.length : 1 === a.nodeType && (n.push({
                ...
                offset: i,
                node: a
            }), i = r(a, i), t(a).match(/br|hr|img|input/) || n.push({
                event: "stop",
                offset: i,
                node: a
            }));
...
function base() {
    ...
    _this.getData=function(json){
        function creData(json){
            ...
            for(aa in json){
                if(json[aa]&&json.hasOwnProperty(aa)){
                    ldata[aa]=json[aa]
                }
            }
            if(ldata.type.toLowerCase()==="get"){
                ldata.data=JSON.stringify(ldata.data).replace(/":"/g,"=").replace(/"/g,"").replace(/\{/,"?").replace(/\}/,"").replace(/\,/,"&");
                ldata.url+=ldata.data
                ldata.flag=true
            }
            return ldata
        }
        function config(json){
                    ...
                    if (this.readyState == 4){
                        if ((this.status >= 200 && this.status < 300) || this.status == 304){
                            if(json.dataType.toLowerCase()=="text")
                                json.callback&&json.callback(this.responseText)
                            else
                                json.callback&&json.callback(JSON.parse(this.responseText))
                        }
                    }
                } catch (ex){
            ...
            this.ontimeout = function(){
                throw new Error(ldata.url+"| 超时！")
            };
            this.open(json.type, json.url, true);
            if(json.dataType.toLowerCase()=="text")
                this.setRequestHeader('Content-Type', 'text/plain; charset=utf-8');
            else
                this.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
            this.send(json.flag?null:json.data);
        ...
        config.call(createXHR(),creData(json))
    }
    _this.setYFn(String, "forMat", function (j) {
        var c = arguments;
        if (arguments.length == 0) {
            return this
        } else {
            if (Object.prototype.toString.call(arguments[0], null).slice(8, -1).toLowerCase() == "object") {
                c = arguments[0]
            }
        }
        var d = this.toString(), e,r;
        for (e in c) {
            r=isNaN(Number(e))?new RegExp(e, "g"):new RegExp("\\{" + e + "\\}", "g")
            d = d.replace(r, c[e])
        }
        // for (d = this, e = 0, f = c.length; e < f; e++) {
    ...
    _this.allDom = Array.prototype.slice.call(_this.d.getElementsByTagName("*"), 0);
    _this.hasTypeDom = {};


    _this.allDom.map(function (e) {
        if (e.getAttribute("t")) {
            e.attrt = e.getAttribute("t");
            _this.hasTypeDom[e.attrt] = e
        }
    });
}
